...
execute if score CONFIG$destroyexplosively DimensionalDoors = BOOL$true DatapackManager as @e[type=minecraft:item,tag=DD_door404,name="Conduit",nbt={Item:{components:{"minecraft:custom_data":{DD_Linked:1b}}}}] at @s align xyz run function dimensionaldoors:effects/destructiveexplosion
execute unless score CONFIG$destroyexplosively DimensionalDoors = BOOL$true DatapackManager as @e[type=minecraft:item,tag=DD_door404,name="Conduit",nbt={Item:{components:{"minecraft:custom_data":{DD_Linked:1b}}}}] at @s at @e[tag=DD_portal,distance=..2,limit=1] run function dimensionaldoors:effects/nondestructiveexplosion