...
execute if score VERSION$minecraft.current DatapackManager matches ..12002 as @e[type=minecraft:item,tag=DD_door404,name="Conduit",nbt={Item:{tag:{DD_Linked:1b}}}] at @s as @e[tag=DD_portal,limit=1,sort=nearest] run tag @s add DD_destroy

# special case for 1.20.3+
execute if score VERSION$minecraft.current DatapackManager matches 12003.. as @e[type=minecraft:item,tag=DD_door404,name="Conduit",nbt={Item:{components:{"minecraft:custom_data":{DD_Linked:1b}}}}] at @s as @e[tag=DD_portal,limit=1,sort=nearest] run tag @s add DD_destroy