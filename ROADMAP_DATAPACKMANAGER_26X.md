...
config option costs ~6–7. A framework selling extensibility cannot charge files per extension. The rebuild
...
- **Fake-player (score-holder) names may stay long and readable.** Evidence (bug MC-74422 [V]) shows
  ...
  full, self-documenting names — **[?]** confirm the score-op path on 1.14–1.17 in-game.
...
the same source. The *commands* are 1.14-safe; the *directory packaging* for 1.14–1.20 is an MPP/build
...
Two mechanisms work, and the rebuild uses both: **`max_format`** (the game refuses a too-new pack; works
against versions that do not exist yet) and a **sentinel leaf** (a probe for the first *unsupported* version
with `forward` one below it). Simulated: 26.2 → `260200` quiet, 26.3 → `260300` alert, hypothetical 27.1 →
`260300` alert. The sentinel degrades safely — if its snapshot-derived block id is renamed, the leaf drops
...
against a primary source; the 26.3 sentinel is snapshot-derived and fails safe.

| Version | Encoded | Probe | Source |
|---|---|---|---|
| 1.21.11 | `12111` | `execute if entity @e[type=minecraft:nautilus]` | 1.21.11 "Mounts of Mayhem", 2025-12-09 |
| 26.1 | `260100` | `execute if block ~ ~ ~ minecraft:golden_dandelion` | 26.1 "Tiny Takeover", 2026-03-24 |
| 26.2 | `260200` | `execute if block ~ ~ ~ minecraft:sulfur` | 26.2 "Chaos Cubed" |
| 26.3 | `260300` | `execute if block ~ ~ ~ minecraft:red_shrub` | 26.3 Snapshot 1, 2026-06-23 — **[?] snapshot** |

Encoding `major*10000 + minor*100 + patch` survives the scheme change (26.1 → `260100` > 1.21.11 → `12111`;
eras never overlap; no 32-bit overflow). **Invariant: each field < 100.** Detection is drop-granular [V]:
1.21.10 and 26.1.1/26.1.2 add no new parseable id, so `.patch` is `0` on 26.x and consumers must not gate on
a hotfix.
...
- **Oracle** — the full 1.14.4 → 26.2 ratchet plus the 26.3 sentinel; the `patch.mcfunction` clobber and
  the dead "too old"/forward machinery are gone.
...
| Q3 | Is `pack.mcmeta` with `pack_format` + `supported_formats` + `min_format`/`max_format` all present accepted across 1.14 → 26.2? | The reference pack's metadata |
| Q4 | Do over-long **fake-player** names drive score operations correctly on 1.14–1.17? (MC-74422 suggests yes; unconfirmed for this path.) | Long readable holder names below 1.18 |
| Q5 | 26.3's final probe id and `pack_format` — `red_shrub` is snapshot-only and the format moved 108→110 mid-snapshot. | Promoting the sentinel to supported |
...
value from the clobbered `12110` to `12109`. Resolve Q1–Q4.
...
**Phase 2 — Delete `datapackmanager-mc26.1/`.** It is dead on arrival: 82 references to the old
`datapackmanager-1.21:` namespace, zero to its own, and a `load.json` bootstrapping a namespace the pack
does not contain [V]. Regenerating from one source beats repairing 82 references.
...
**Phase 4 — Promote 26.3 at GA.** Re-verify `red_shrub` against the release changelog, raise `max_format`,
move the sentinel to the next unsupported version. Pinning `max_format` at 26.2 means consumers visibly go
incompatible on 26.3 launch day rather than silently mis-detecting — the correct trade, but it makes the 26.3
turnaround a release commitment.
