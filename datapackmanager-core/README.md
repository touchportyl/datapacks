...
1.14–1.20 needs a plural-layout build of the same source. See **[LIMITATIONS.md](LIMITATIONS.md)** for
...
CLI only touches players who have actually used the trigger.
...
| `VERSION$minecraft.backward` / `.forward` | Supported range (11404 … 260299) |

26.1 encodes as `260100`, sorting above 1.21.11's `12111`; old-scheme values are ≤ `19999` and
new-scheme ≥ `260000`, so the eras never overlap. **Each field stays below 100** — `26.1.100` would
...
- **Drop granularity only.** 1.21.10 added no new parseable id over 1.21.9, and 26.1.1 / 26.1.2 add
  none over 26.1 — those are indistinguishable at runtime, so `.patch` is `0` on 26.x. Never gate on
  a hotfix.
- **"Too new" is handled two ways.** `pack.mcmeta`'s `max_format` is the real guard — the game
  refuses a pack whose format is beyond it, and that works against versions that do not exist yet.
  The `26.3` **sentinel leaf** is a runtime backstop: it is a probe for the first *unsupported*
  version, with `forward = 260299`, so 26.3 and anything above it trip the alert. Simulated:
  26.2 → `260200` (quiet), 26.3 → `260300` (alert), a hypothetical 27.1 → `260300` (alert). If
  `red_shrub` is renamed before 26.3 ships, the leaf fails to parse and drops — detection falls back
...
1. Load **this artifact** on 1.21, 26.1, and 26.2; `/trigger dm_command set 3` should report each
   correctly. It will **not** load below 1.21 (singular `function/` layout). To test the 1.14.4 floor,
...
3. Load on a 26.3 snapshot and confirm the too-new alert fires and the pack still runs.
...
5. Confirm long fake-player names (e.g. `VERSION$minecraft.current`, 25 chars) drive scoreboard
   operations correctly on 1.14–1.17. Evidence (bug MC-74422) says over-long fake-player names are
...
were each verified against a primary source: `wooden_spear` (1.21.11 "Mounts of Mayhem", 2025-12-09),
`golden_dandelion` (26.1, 2026-03-24), `sulfur` (26.2), and `red_shrub` (26.3 Snapshot 1, 2026-06-23
— **snapshot only, may be renamed**). All are block/item/particle probes, not entity-type selectors,
because unknown entity types do not reliably parse-fail before 1.21.2 (see LIMITATIONS.md §5.3).
