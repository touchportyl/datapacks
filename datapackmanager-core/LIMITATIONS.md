...
1.14.4-safe (sections below), but delivering a pack that actually **loads** on 1.14 to 1.20.6 requires a
...
1.14.4 to 1.20.5 are inert in this artifact (they run only in a plural-layout build).
...
`min_format`/`max_format` at once: 1.21 to 1.21.8 read the first two and ignore the rest; 1.21.9+ read
...
The framework's components sit in the JSON and SNBT safe set and parse on both sides.
...
1.21+ artifact. In a plural-layout build for 1.14 to 1.20.4 the icon must become `{"item":...}`. `root.json` is a
purely cosmetic toast-group parent, and the functional enable-on-join advancement
(`data/datapackmanager-2/advancement/enable_command.json`) has **no display at all**, so nothing operational
depends on this. **[unverified]** whether a bad icon fails only the
...
fails to load the **entire tag**. The framework's own tag entries all resolve (`command.json` lists
`datapackmanager-2:root/command/router`, which exists; the event tags ship empty), so it is safe. A subscriber that lists a missing handler will drop the whole
...
overlays themselves need pack_format >= 16 (1.20.2), so they cannot help 1.14 to 1.20.1, which need the plural
...
`VERSION$minecraft.current` (25 chars) exceed 16. **[unverified]** for the score-op path on 1.14 to 1.17;
...
`4` = 1.14 to 1.14.4, `5` = 1.15 to 1.16.1, `6` = 1.16.2 to 1.16.5, `7` = 1.17 to 1.17.1, `8` = 1.18 to 1.18.1,
`9` = 1.18.2, `10` = 1.19 to 1.19.3, `12` = 1.19.4, `15` = 1.20 to 1.20.1, `18` = 1.20.2, `26` = 1.20.3 to 1.20.4,
`41` = 1.20.5 to 1.20.6, `48` = 1.21 to 1.21.1, `57` = 1.21.2 to 1.21.3, `61` = 1.21.4, `71` = 1.21.5, `80` = 1.21.6,
`81` = 1.21.7 to 1.21.8, `88.0` = 1.21.9 to 1.21.10, `94.1` = 1.21.11, `101.1` = 26.1 to 26.1.2, `107.1` = 26.2,
`121.0` = 26.3, `122.0` = 26.4 Snapshot 1 (moving).
...
real "this Minecraft is too new" guard (see the too-new note in the README; no sentinel leaf exists today).
...
text-SNBT boundary lands on a leaf and the framework's usage is JSON and SNBT safe.
...
## 8. Version number encoding for the year.drop scheme

Minecraft moved from `1.21.11` to `26.1` on 2026-03-24 and now numbers releases year.drop.hotfix. The oracle keeps
its five-digit `major*10000 + minor*100 + patch` value by storing those releases as 1.<year>.<drop>: 26.1 is
`12601`, 26.2 is `12602`, 26.3 is `12603`. Consequences, all deliberate:

- `VERSION$minecraft.current.major` stays `1`, `.minor` carries the year (26) and `.patch` the drop (3). Every
  consumer gate written as `.minor matches N..` therefore stays true on 26.x, and `.minor matches 14..15`
  style ranges stay false, which is what the consumer packs rely on.
- Hotfixes (26.1.1, 26.1.2) add no parseable id and report as their drop; nothing may gate on a hotfix.
- The in-game text prints `26.3` for `current matches 12600..` and `1.21.9` style below it; both the
  `minecraft_version` alert and the `version` verb carry the two variants.
- The scheme runs out at year 99 (`19999`), which is not a practical concern; the `260100` style of an
  earlier draft was rejected because it would have put the drop in `.minor` and broken the consumer gates.
