...
# 5. clamp speed value to [default, max]
# use max speed depending on leash status
execute as @n[tag=SG_ctx_vehicle,tag=!SG_leashed] \
  if score @s SwiftGhasts > VAR$speed.max SwiftGhasts \
  run scoreboard players operation @s SwiftGhasts = VAR$speed.max SwiftGhasts
execute as @n[tag=SG_ctx_vehicle,tag=SG_leashed] \
  if score @s SwiftGhasts > VAR$speed.max.leashed SwiftGhasts \
  run scoreboard players operation @s SwiftGhasts = VAR$speed.max.leashed SwiftGhasts

execute as @n[tag=SG_ctx_vehicle] if score @s SwiftGhasts < VAR$speed.default SwiftGhasts \
  run scoreboard players operation @s SwiftGhasts = VAR$speed.default SwiftGhasts
