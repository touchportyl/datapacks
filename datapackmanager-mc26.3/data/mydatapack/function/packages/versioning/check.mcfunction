...
# one line per released version, oldest first; each update file sets VERSION$datapack.latest itself
...
execute if score VERSION$minecraft.current DatapackManager < VERSION$minecraft.backward MyDatapack if score VERSION$ignorecompatibility.backward MyDatapack = BOOL$false DatapackManager run function mydatapack:packages/versioning/throw/datapacknotcompatible
execute if score VERSION$minecraft.current DatapackManager > VERSION$minecraft.forward MyDatapack if score VERSION$ignorecompatibility.forward MyDatapack = BOOL$false DatapackManager run function mydatapack:packages/versioning/throw/datapacknotcompatible
