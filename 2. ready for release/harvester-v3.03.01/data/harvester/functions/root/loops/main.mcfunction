...
# damage tool (only works in 1.17+ because of the /item command)
# this runs at the end because we want to ensure all other actions are completed first
execute if score VERSION$minecraft.current.minor DatapackManager matches 17.. if score CONFIG$toolbreaking Harvester = BOOL$true DatapackManager as @a[gamemode=survival,tag=HV_success] at @s run function harvester:root/damagetool
execute if score VERSION$minecraft.current.minor DatapackManager matches 17.. if score CONFIG$toolbreaking Harvester = BOOL$true DatapackManager as @a[gamemode=adventure,tag=HV_success] at @s run function harvester:root/damagetool
