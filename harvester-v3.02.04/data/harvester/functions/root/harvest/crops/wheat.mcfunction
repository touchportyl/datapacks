...
execute if entity @s[tag=HV_wheat] if score CONFIG$wheat.xp Harvester = BOOL$true DatapackManager run summon experience_orb ~ ~ ~ {Age:5960,Health:1,Value:1}
execute if score RAND$4 Harvester matches 1 if score CONFIG$wheat.xp.bonus Harvester = BOOL$true DatapackManager run summon experience_orb ~ ~ ~ {Age:5960,Health:1,Value:1}
