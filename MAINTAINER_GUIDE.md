...
Both packs are consumers of this framework, following an identical contract. **Learn the framework once (sections 3–6) and 80% of both packs becomes legible.** The gameplay code (sections 7–8) is comparatively small and sits on top.
...
| `datapackmanager-26.1` | The next-gen framework (in `datapackmanager-mc26.1/`), mid-migration — see §11. |
...
`load` does **not** do version detection. That happens in `root/core/initialize`, run ~1–2 s later via the deferred event.
...
3. Sets MC version bounds: `VERSION$minecraft.backward = 11404`, `VERSION$minecraft.forward = 12110`.
...
- Harvester: `HV_damagetool`, `HV_sneak_time` (vanilla criterion `minecraft.custom:minecraft.sneak_time`, only on MC 1.14–1.15).
...
Player config uses `/trigger` objectives `z_<pack>_NNN_{t,f}`: `z_` sorts them to the bottom of tab-complete, `NNN` is a zero-padded setting id, `_t`=set-true/`_f`=set-false. One objective per (setting × direction). A poll loop reads them into `CONFIG$` fake-players and re-arms. See §8-adjacent detail in the Harvester dive (§7.7).

---

## 5. Language quirks & idioms

**(a) Version-integer encoding.** Versions are packed to sortable ints: `major*10000 + minor*100 + patch`. 1.21.9 → `12109`, 1.19.4 → `11904`. Datapacks use the same: v3.03.02 → `30302`. `minor` is *also* stored separately as a small int (`.current.minor = 21`) for range gates like `matches 14..15`. Comparisons: `matches 11904..` (≥1.19.4), `matches ..12005` (≤1.20.5), `matches 11904..12000`.
...
Both packs run on Minecraft 1.14 through 26.1 from a single codebase. This is achieved by a runtime version oracle plus a small set of dispatch idioms. Master this before touching anything.
...
**Canary catalogue** (first line of each `versions/*.mcfunction`):
...
**`patch.mcfunction`** has *no* canary — it runs `if current matches 12109..` and unconditionally sets `12110` (1.21.10). This is how a patch release with no unique detectable feature gets represented. **Quirk: 1.21.9 and 1.21.10 are indistinguishable to the engine (both report `12110`).**

### 6.3 Bounds & activation

`initialize` sets `VERSION$minecraft.backward = 11404` (oldest supported) and `VERSION$minecraft.forward = 12110` (newest known). Then `check` emits alerts and sets activation:

- `current < backward` → `alerts/minecraftnotcompatible` (red, hard-disable).
- `current > forward` → `alerts/minecraftunknownversion` (gold warning, still runs).
...
In MC 1.16–1.20, `setblock replace` on an open door desynced/popped the two halves, so those versions need the v2 destroy-and-rebuild workaround. 1.14–1.15 and **1.21+** use the simple v1 `replace`. **Key lesson: version behavior can regress and later be restored — `21..` deliberately reuses the *older* v1 codepath. Never assume forks are monotonic.**
...
- `.mpp-datapack` holds `packFormatVersionMin/Max` (floats; `.1` = snapshot format). Harvester: `15..94.1` (1.14→26.1). **Dimensional Doors: `48..48` — narrow and inconsistent with its 1.14+ runtime support; likely stale, widen it.** Template `datapackmanager-mc26.1`: `15..101.1`.
...
- `createlinkedconduit.mcfunction` has a harmless gap `12007..12099` (no branch for nonexistent 1.20.7–1.20.99).
...
**Per-wood:** `setblock` requires a *literal* block id + full blockstate — you cannot parameterize the wood. So each wood needs its own file enumerating all 8 facing×hinge combos. `summon/door.mcfunction` also assigns each wood a hardcoded `DD_particle_X` window class. **Adding a new wood door means editing ≥3 places:** `summon/door/<ver>.mcfunction` (portal + particle class), `closermechanism/v1/<wood>` + `v1/<ver>` dispatch, and `closermechanism/v2/<wood>` + `v2/<ver>` dispatch. Miss any and the door won't portal or won't close.
...
**Why v1 vs v2 closer:** see §6.6 — v2 (1.16–1.20) destroys the lower half and rebuilds the upper (plus `kill @e[…name="Oak Door"…Age:0s]` to remove the popped item); v1 (1.14–1.15, 1.21+) does a clean `setblock replace`.
...
`destroy/filter` tags portals whose block is no longer a door `DD_destroy`, propagates to all doors sharing the `DD_doorID` (using `sort=furthest` — **not `distance=`, which would filter to same dimension**), runs `handler` (explode via `effects/destructiveexplosion` or `nondestructiveexplosion` per `CONFIG$destroyexplosively`, `forceload remove`, kill marker). Fireball summon NBT is version-split (`{direction,Fire}` 1.14–15 vs `{Motion,Invulnerable}` 1.16+).

### 8.7 Effects & version splits

`flash` particle gained a mandatory `color` field at 1.21.9 → `12109` split everywhere (`compatibility/flash` vs `flash-1.21.9`). `sonic_boom` only ≥1.19 (else `explosion` fallback). Ambience particle volume increases on 1.14–15 (underwater particles vanish instantly there).
...
**Known bug in migrations:** every `updates/vX` calls `datapackmanager-1.21:packages/effects/ui_jingle`, but the real function is `packages/effects/ui/jingle` (slash, not underscore) — a silent dead reference. Use the slash form in new migrations.
...
Concrete ordered checklist. Say the new version is **1.22.0 = `12200`**. Do every framework step in the **canonical template**, then propagate to **every embedded copy** of `datapackmanager-1.21` (Harvester + DD + any other pack) so they stay byte-identical (see step 8).
...
**2. Create `packages/versioning/versions/1.22.0.mcfunction`** (match the actual first-stable release string, mirroring `1.21.9` naming):
```mcfunction
execute if block ~ ~ ~ minecraft:<new_block_from_1.22>       # canary, line 1
scoreboard players set VERSION$minecraft.current       DatapackManager 12200
scoreboard players set VERSION$minecraft.current.major DatapackManager 1
scoreboard players set VERSION$minecraft.current.minor DatapackManager 22
scoreboard players set VERSION$minecraft.current.patch DatapackManager 0
```
The canary **must be line 1**, before the `set`s, or detection silently misreports. Never add non-probe logic to a `versions/*` file.

**3. Register it in `check.mcfunction`** (the `datapackmanager-1.21` MC-version copy — *not* the pack save-data copy, §6.2/§9), inserted in ascending order just before the `patch` line:
```mcfunction
execute unless score VERSION$minecraft.current DatapackManager matches 12200.. run function datapackmanager-1.21:packages/versioning/versions/1.22.0
```
Re-point the trailing `patch` guard (`unless … matches 12200..` / `if … matches 12200..`) and update `patch.mcfunction`'s numbers if 1.22 becomes the newest branch.

**4. Bump forward bounds.** In `datapackmanager-1.21/…/root/core/initialize.mcfunction` set `VERSION$minecraft.forward … 12200`. In each pack's declare file (`harvester/packages/preinstaller/check.mcfunction`, DD equivalent) bump `VERSION$minecraft.forward <PackId>` and decide `ignorecompatibility.forward` (Harvester is already `true`).
...
**8. Propagate framework changes from the canonical template, then verify byte-identical copies.** There is one source of truth for the framework: the standalone template directory **`datapackmanager-mc1.21/`** at the repo root (the full reference implementation; the Project Creator / Framework Updater treat it as canonical). **Do NOT hand-edit the vendored `data/datapackmanager-1.21` copies first**, and do NOT edit one pack's copy and call it done. Workflow:
  - a. Make every framework edit (steps 1–4, 6) in the canonical template `datapackmanager-mc1.21/` only.
...
## 11. Framework namespace migration (`datapackmanager-1.21` → `datapackmanager-26.1`)

A next-gen framework exists at `datapackmanager-mc26.1/` with a new namespace `datapackmanager-26.1`, but the migration is **mid-flight and unfinished**: that template's `load.json` still points at `datapackmanager-1.21:root/core/load`. The `-1.21` suffix is the intended version boundary — to bump the library to an incompatible generation you copy the folder, rewrite every `datapackmanager-1.21:` reference inside it, in `load.json`, and in every consuming pack, then re-vendor the copy into each pack under `data/`. This is a global namespace-prefix rename plus re-vendoring; do it with the Python Framework Updater (from the canonical template, per §10 step 8), not by hand.
...
12. **Known dead references (safe to clean up):** `root/core/uninstall` calls nonexistent `root/commands/config-`/`uninstall-` (refactor leftovers); every `updates/vX` calls `packages/effects/ui_jingle` instead of `ui/jingle`; `predicate/player_checkers/is_sneaking_.json` is unused. Missing-function calls are silent no-ops, so these are harmless but should be removed.
...
15. **`patch.mcfunction` makes 1.21.9 and 1.21.10 indistinguishable** (both `12110`). If you ever need to branch between them, you can't without adding a real canary for 1.21.10.
