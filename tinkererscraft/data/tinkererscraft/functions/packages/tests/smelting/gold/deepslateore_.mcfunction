...
execute if score $tc.temp.doubler.deepslate.alert TC_temp_tests matches 0 run tellraw @a [{"text":"\n> Deepslate ore is set to produce ","color":"gray"},{"score":{"name":"$tc.gold.output","objective":"TC_temp_tests"}},{"text":" liquid metal in the config file."}]
execute if score $tc.temp.doubler.deepslate.alert TC_temp_tests matches 0 run scoreboard players set $tc.temp.doubler.deepslate.alert TC_temp_tests 1
...
# schedule next
schedule function tinkererscraft:packages/tests/smelting/gold/netherore 1s
