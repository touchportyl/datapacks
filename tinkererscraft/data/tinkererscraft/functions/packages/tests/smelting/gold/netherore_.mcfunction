...
execute unless score $tc.gold.skipped TC_temp_tests matches 1.. if score $tc.doubler.nethergold.alert TC_temp_tests matches 0 run tellraw @a [{"text":"\n> Nether gold ore is set to produce ","color":"gray"},{"score":{"name":"$tc.gold.output","objective":"TC_temp_tests"}},{"text":" liquid metal in the config file."}]
execute unless score $tc.gold.skipped TC_temp_tests matches 1.. if score $tc.doubler.nethergold.alert TC_temp_tests matches 0 run scoreboard players set $tc.doubler.nethergold.alert TC_temp_tests 1
...
# schedule next
schedule function tinkererscraft:packages/tests/smelting/gold/ingot 2t
