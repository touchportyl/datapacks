...
execute unless score @e[tag=TC_temp_tests_smelting,sort=nearest,limit=1] TC_lCopper matches 0 run tellraw @a [{"text":""},{"text":"\n> Bug detected! ","color":"red"},{"text":"Smelting weathered cut copper slab should produce 0 liquid copper but it produces "},{"score":{"name":"@e[tag=TC_temp_tests_smelting,sort=nearest,limit=1]","objective":"TC_lCopper"}},{"text":" instead."}]
...
# schedule next
schedule function tinkererscraft:packages/tests/smelting/copper/oxidizedslab 1s
